...
public:
    ...
    public:
        ...
        {
            if (dim == 2)
                values[0] = 4*u_m*(p[1]*(H-p[1])/(H*H)+offset)*std::sin(M_PI*get_time()/4.0);    // 2D
            else if (dim == 3)
        ...
        value(const Point<dim> &p, const unsigned int component = 0) const override
        {
            if (component == 0){
                if (dim == 2)
                    return 4*u_m*(p[1]*(H-p[1])/(H*H)+offset)*std::sin(M_PI*get_time()/4.0);    // 2D
                else if (dim == 3)
        ...
        {
            if (dim == 2)
                return  4.0*u_m*(1.0/6.0 + offset)*std::sin(M_PI*get_time()/4.0);                     //(2.0/3.0)*u_m*std::sin(M_PI*get_time());    // 2D
            else if (dim == 3)
    ...
    public:
        ...
        vector_value(const Point<dim> & /*p*/, Vector<double> & values) const override
        {
            values[0] = 4*u_m*std::sin(M_PI*get_time()/4.0)*offset;    // 2D
        ...
        {
            if (component == 0)
                return 4*u_m*std::sin(M_PI*get_time()/4.0)*offset;    // 2D
            else    
                return 0.0;
        }
