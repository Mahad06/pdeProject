...
NavierStokes::output(const unsigned int &time_step, const double &time) const
{
  ...
  // output_file_name = "/scratch/hpc/par5/results/output-" 
...
NavierStokes::solve()
{
  ...
  while (time < T)
    {
      ...
      lift_force.push_back(forces[0]);
      drag_force.push_back(forces[1]);
      ReynoldsNumber.push_back( inlet_velocity.mean_value()*0.1/nu  );
      lift_coeff.push_back(  2.0*lift_force.back() / (inlet_velocity.mean_value()*inlet_velocity.mean_value()*0.1)  );
      drag_coeff.push_back(  2.0*drag_force.back() / (inlet_velocity.mean_value()*inlet_velocity.mean_value()*0.1)  );
    }
  ...
  for(int i=0;i<(int)lift_force.size();i++)
    {
        coeff << timeVector[i] << "," << ReynoldsNumber[i] << "," << lift_force[i] << "," <<
         lift_coeff[i] << "," << drag_force[i] << "," << drag_coeff[i] << std::endl;
    }
